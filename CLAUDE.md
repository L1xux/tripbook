...
- **백엔드**: Python 3.11+, FastAPI, SQLAlchemy 2.0, SQLite, openai SDK, httpx, Pillow, qrcode, pytest
- **프론트엔드**: React 19 + TypeScript + Vite, react-router-dom, vitest (v2 프론트는 별도 계획(Plan B))
- **외부 API**: OpenAI(`gpt-4o-mini` — 사진 감정 제안 + 캡션 편집 + 감정 아크, Whisper `whisper-1` — 음성 인식),
  Sweetbook Book Print(TEMPLATE 방식). LLM은 OpenAI로 통일(anthropic 제거).
...
- 캡션 편집·감정 제안·감정 아크 `gpt-4o-mini`(OpenAI), 음성 인식 Whisper `whisper-1`.
- 비전 감정 제안은 `response_format` json_schema(strict)로 `{scene, suggested_emotion}` 강제.
- API 키는 `.env`로만: `OPENAI_API_KEY`(LLM+Whisper), `SWEETBOOK_API_KEY`, `SWEETBOOK_ENV=sandbox|live`, `PUBLIC_WEB_BASE`.
...
- **캡션 불변식(창작 금지)**: 음성 캡션은 사용자가 말한 원문을 다듬을 뿐 새 사실·감정·인물·장소를
  추가하지 않는다(`app/ai/caption.py:NO_INVENTION`). 편집이 실패하면 인식된 원문을 그대로 캡션으로 보존한다
  (침묵하거나 지어내지 않는다).
